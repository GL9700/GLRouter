...
- (IBAction)onPush:(UIButton *)sender {
    NSLog(@"willPush----%@", self);
    [Router openURL:@"GLRT://push/GLPublicViewController?content=this Is Push&message=back" form:self before:^(id targetController) {
...
//    NSString *url = @"GLRT://push/GLXIBPublicViewController?tipInfo=你好，你需要登录&imgURL=https://www.baidu.com/img/flexible/logo/pc/result@2.png";
    ...
    [Router openURL:url form:self before:^BOOL(id targetController) {
        @weak(targetController)
        [targetController setValue:^{
            @strong(targetController)
            [((UIViewController *)targetController).navigationController popViewControllerAnimated:YES];
...
//    [Router openURL:@"GLRT://invoke/GLFuncTools?method=toastTime" form:nil before:nil after:nil];
    ...
    [Router openURL:@"GLRT://invoke/GLFuncTools?method=whatistheTime" form:nil before:nil after:^(id ret) {
        NSLog(@"ret:%@", ret);
...
- (IBAction)onInvokeMessage:(UIButton *)sender {
    [Router openURL:@"GLRT://invoke/GLFuncTools?method=toastMessage:&msg=从前有座⛰️，山里有个🏠，🏠里有个👨‍🦲" form:nil before:nil after:nil];
}

- (IBAction)onInvokeCount:(UIButton *)sender {
    [Router openURL:@"GLRT://invoke/GLFuncTools?method=toastCountA:B:C:&a=3&b=5&c=7" form:nil before:nil after:^(id ret) {
        aswer.text = [NSString stringWithFormat:@"%@" , ret];
    }];
}

- (IBAction)onPushWeb:(UIButton *)sender {
    [Router openURL:@"GLRT://push/GLWebViewController?url=http://www.jianshu.com" form:self before:nil after:nil];
}
